...
let package = Package(
  ...
  dependencies: [
    ...
    // actually written and tested against: **1.7.0 pins 0.33.2** in its own `Package.resolved`.
    //
    ...
    // convention: 0.33.x patches stay allowed, and 0.34+ requires a deliberate, tested upgrade of this fork
    // (rebasing onto an upstream tag that supports it).
    .package(
      url: "https://github.com/pointfreeco/swift-structured-queries",
      .upToNextMinor(from: "0.33.2"),
      traits: [
        .trait(
          name: "LazyInitializableByDefault",
