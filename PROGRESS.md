...
- **Status:** `milestone-reached` — every box on the roadmap is checked. Patches 1–19 sit on `mango/patches-1.10`, the adopted consumer base (and on `mango/patches-1.12`); the next work here is consumer-driven (a new incident, or a retarget onto the next upstream tag).
- **Updated:** 2026-09-17
...
- **Build:** green · **Tests:** green — **369 tests, ZERO failures, 15 known issues** on `mango/patches-1.10` (2026-09-26, macOS) + the iOS `AppLifecycleTests` (3) on a simulator · **Simulator-verified:** the lifecycle tests only
...
>
> ⚠ Patches 15, 16 and 17 are **inert for MonteSprout until it bumps its pin** (still `d84eeaa`,
> the patch-14 tip) — that bump is `/mango-update`'s job in that repo (its 61.2, through MangoSync
> 0.9.2), never a side effect of work here.
>
